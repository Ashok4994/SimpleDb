...
public class HeapPage implements Page {
	...
	public Iterator<Tuple> iterator() {
		// some code goes here
		ArrayList<Tuple> Tup_arr = new ArrayList<>();
		int count=this.entryCount();
		for(int i=0;i<count;i++) {
			System.out.println(getTuple(i));
			if(getTuple(i)!=null){
				System.out.println(getTuple(i));
				Tup_arr.add(getTuple(i));
			}
		}

		return Tup_arr.iterator();
	...
	/**
	 * Marks this {@code HeapPage} as dirty/not dirty and  that transaction that did the dirtying
	 */
	...
	public Tuple getTuple(int entryID) {
		...
		System.out.println("Location : "+loc + " "+entryID);
		Tuple tuple = createTuple(new DataInputStream(new ByteArrayInputStream(data, loc, data.length-loc)));
	...
	protected Tuple createTuple(DataInputStream in) {
		try {
			...
			for (int j = 0; j < td.numFields(); j++) {
				Field f = (Field) td.getType(j).parse(in);
				t.setField(j, f);
			}
			return t;
